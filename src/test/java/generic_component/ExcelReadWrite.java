...
public class ExcelReadWrite {
	...
	public String Readvalue(XSSFSheet Sheet,int rowIndex, int colnum)
	{
		XSSFCell cell = Sheet.getRow(rowIndex).getCell(colnum);
	...
	public void writecell(XSSFSheet Sheet,int rowIndex,int colnum, String value )
	{
		//writing the cell
		XSSFCell writecell = Sheet.getRow(rowIndex).getCell(colnum);
		if(writecell==null)
		{
			writecell=Sheet.getRow(rowIndex).createCell(colnum);
		}
		
		writecell.setCellValue(value);
	}
	
	public void writecell(XSSFSheet Sheet,int rowIndex,String colname, String value ){
		int colindex = 0;
		for(int i=0;i<getcolcount(Sheet,0);i++){
